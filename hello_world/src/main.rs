...
fn main() {
   let mut vrr:Vec<&str> = vec!["abc", "def", "ghi"];   // vrr is the current owner of the vector

   write_vrr( &mut vrr);  // vrr ownership is transfered to the function
   println!("Strings in the vector is : {:?}",vrr); // Ownership transfered by reference and borrowed back to the caller

}
fn write_vrr(in_vrr:  &mut Vec<&str>) {   // in_vrr becomes the new owner of the vector
